...
Hunt's primitives — sealed inference, multi-specialist competition, on-chain per-domain reputation — are not actually about smart contracts. They are about **verifiable AI on private data.** Three non-crypto verticals are positioned for v2, each a distinct face of the same primitive against a different counterparty:

- **Pillar 5a — insurance-claim-denial defense** (defends citizens *against* private-payor opaque AI). Lead vertical; framing below.
