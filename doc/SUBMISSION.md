...
| [`packages/mcp-server`](https://github.com/Ridwannurudeen/hunt/tree/master/packages/mcp-server) | **MCP server for Claude Desktop, Cursor, any MCP client.** 10 tools wrap the same primitives (`hunt_stats`, `hunt_list_hunters`, `hunt_verify_bounty`, `hunt_get_class_rep`, `hunt_canonical_digest`, etc). AI agents query Hunt natively. `npx -y @hunt-protocol/mcp-server` once published. |
| [`packages/sdk`](https://github.com/Ridwannurudeen/hunt/tree/master/packages/sdk) | **npm SDK** `@hunt-protocol/verifiable-ai` — `findingDigest`, `classToBytes32`, `signAttestation`, `verifyAttestation`, ECIES helpers. 9 tests passing. 5 example scripts (smart-contract / insurance / medical / benefits / generic-classification). |
