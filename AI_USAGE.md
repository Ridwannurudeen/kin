...
- 230 tests written and passing — the 212-test baseline (`test/Hunt.test.js`, `test/Kin.test.js`, `test/verifier.test.js`, `test/ecdh.test.js`, `test/embedding.test.js`, `test/Notary.test.js`, `test/HuntReputationOracle.test.js`, `test/pubkey.test.js`, `test/verify_bounty.test.js`, `test/review.test.js`) plus +18 added in the 2026-05-16 security-hardening pass: `test/Hunt.test.js` expanded with input-validation tests for the new `MAX_SCOPE_CWES` / `MAX_SELF_EVAL_BPS` / empty-class guards in `contracts/Hunt.sol`; new `test/abi.test.js` pins `deployments/Hunt.json` + `public/contracts.js` to the deployed-bytecode storage width; new `test/inference.test.js` covers the SSRF + DNS-rebinding defense in `lib/inference.js`. Two Kin v2 agent test files target the older `review.summary` schema and are parked under `test-legacy/`, excluded from the default `npm test`.
