...
const { handle, verifierWallet, checkBar, buildCredential } =
  await import("../verifier/server.js");
const { credentialDigest, recoverWalletClaim, buildWalletClaimMessage } =
  await import("../lib/credential.js");
...
describe("verifier — setup", () => {
  ...
  it("reflects only allowed browser origins", async () => {
    const allowed = await get("/health", {
      headers: { Origin: "https://hunt.gudman.xyz" },
    });
    ...
    const blocked = await get("/health", {
...
describe("verifier — /verify/start + /verify/finish (STUB_MODE)", () => {
  it("start returns ticket + login + claimMessage", async () => {
    const { status, json } = await post("/verify/start", {
      code: "fake",
  ...
  it("finish returns credential after wallet sig over claim", async () => {
    const { json: started } = await post("/verify/start", {
      code: "fake",
      redirectUri: "http://localhost/cb",
    });
  ...
  it("finish rejects malformed wallet before credential signing", async () => {
    const { json: started } = await post("/verify/start", {
      code: "fake",
      redirectUri: "http://localhost/cb",
    });
    const walletSig = await ethers.Wallet.createRandom().signMessage(
      started.claimMessage,
    );
    const { status, json } = await post("/verify/finish", {
      ticket: started.ticket,
      wallet: "0xabc",
    ...
    assert.equal(status, 400);
    assert.match(json.error, /wallet invalid/);
  });

  it("finish rejects malformed walletSig as a bad request", async () => {
    const { json: started } = await post("/verify/start", {
      code: "fake",
      redirectUri: "http://localhost/cb",
    });
    ...
    assert.equal(status, 400);
    assert.match(json.error, /walletSig invalid/);
  });
  ...
  it("tickets are single-use (second consume fails)", async () => {
    ...
    const userWallet = ethers.Wallet.createRandom();
    const walletSig = await userWallet.signMessage(started.claimMessage);
    const first = await post("/verify/finish", {
      ticket: started.ticket,
      wallet: userWallet.address,
      walletSig,
    });
    assert.equal(first.status, 200);
    const second = await post("/verify/finish", {
      ticket: started.ticket,
      wallet: userWallet.address,
      walletSig,
    });
...
describe("verifier — /admin/issue", () => {
  it("issues credential with valid admin key + above bar", async () => {
    ...
    const recovered = ethers.verifyMessage(
      ethers.getBytes(credentialDigest(wallet, json.credential)),
      json.credential.sig,
    );
    assert.equal(recovered, VERIFIER.address);
  });
  ...
  it("rejects missing wallet or login", async () => {
    const r1 = await post("/admin/issue", {
      adminKey: "test-admin-key",
      login: "x",
      accountAgeDays: 1500,
      mergedPRs: 80,
    ...
    assert.equal(r1.status, 400);
    const r2 = await post("/admin/issue", {
      adminKey: "test-admin-key",
      wallet: "0xabc",
      accountAgeDays: 1500,
      mergedPRs: 80,
  ...
  it("rejects malformed admin numeric fields", async () => {
    const wallet = ethers.Wallet.createRandom().address;
    const { status, json } = await post("/admin/issue", {
      adminKey: "test-admin-key",
      wallet,
      login: "x",
      accountAgeDays: "1500.5",
      mergedPRs: 80,
      codeReviewCount: 40,
    });
    assert.equal(status, 400);
    assert.match(json.error, /accountAgeDays invalid/);
  });

  it("rejects oversized JSON bodies", async () => {
    const res = await fetch(`${baseUrl}/admin/issue`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        adminKey: "test-admin-key",
        wallet: ethers.Wallet.createRandom().address,
        login: "x".repeat(70000),
      }),
    });
    const json = await res.json();
    assert.equal(res.status, 400);
    assert.match(json.error, /body too large/);
  });
});
