...
describe("Hunt — deployment", () => {
  it("sets teeSigner, verifier, admin from constructor", async () => {
    ...
    assert.equal(
      (await env.hunt.teeSigner()).toLowerCase(),
      env.teeSigner.address.toLowerCase(),
    );
    assert.equal(
      (await env.hunt.verifier()).toLowerCase(),
      env.verifier.address.toLowerCase(),
    );
    assert.equal(
      (await env.hunt.admin()).toLowerCase(),
      env.admin.address.toLowerCase(),
    );
  });
...
describe("Hunt — mintHunter happy path", () => {
  it("mints, emits HunterMinted, persists fields", async () => {
    const env = await deployHunt();
    const { hunterId, sampleRoots, embedRoots } = await mintHunterFor(
      env,
      env.user,
      {
  ...
  it("increments nextHunterId across multiple mints", async () => {
    ...
    await mintHunterFor(env, env.client, {
      githubHandleHash: env.ethers.keccak256(
        env.ethers.toUtf8Bytes("different-hunter"),
      ),
    });
...
describe("Hunt — mintHunter validation", () => {
  it("reverts when samples < 3", async () => {
    const env = await deployHunt();
    await assert.rejects(
      mintHunterFor(env, env.user, {
        sampleRoots: rootList(env.ethers, 2),
        embedRoots: rootList(env.ethers, 2),
      }),
      /samples 3\.\.20/,
    );
  });
  ...
  it("reverts when samples > 20", async () => {
    ...
    await assert.rejects(
      mintHunterFor(env, env.user, {
        sampleRoots: rootList(env.ethers, 21),
        embedRoots: rootList(env.ethers, 21),
      }),
      /samples 3\.\.20/,
    );
  });
  ...
  it("reverts on sample/embed length mismatch", async () => {
    ...
    await assert.rejects(
      mintHunterFor(env, env.user, {
        sampleRoots: rootList(env.ethers, 3),
        embedRoots: rootList(env.ethers, 4),
      }),
  ...
  it("reverts on empty specialty", async () => {
    const env = await deployHunt();
    await assert.rejects(
      mintHunterFor(env, env.user, { specialty: "" }),
      /specialty 1\.\.64/,
    );
  });
  ...
  it("reverts when credential.verifier != contract verifier", async () => {
    const env = await deployHunt();
    const wrongVerifier = env.ethers.Wallet.createRandom().connect(
      env.ethers.provider,
    );
    ...
    await assert.rejects(
      env.hunt
        .connect(env.user)
        .mintHunter(cred, sampleRoots, embedRoots, fp, "reentrancy", "d"),
      /wrong verifier/,
    );
  });
  ...
  it("reverts on credential reuse", async () => {
    const env = await deployHunt();
    const handleHash = env.ethers.keccak256(
      env.ethers.toUtf8Bytes("reused-hunter-handle"),
    );
    const verifiedAt = 1_700_000_000;
    await mintHunterFor(env, env.user, {
      githubHandleHash: handleHash,
      credOverrides: { verifiedAt },
    });
    await assert.rejects(
      mintHunterFor(env, env.user, {
        githubHandleHash: handleHash,
...
describe("Hunt — pauseHunter", () => {
  ...
  it("non-owner reverts", async () => {
    ...
    await mintHunterFor(env, env.user);
    await assert.rejects(
      env.hunt.connect(env.client).pauseHunter(0, true),
      /not owner/,
    );
  });
});
...
describe("Hunt — postBounty happy path", () => {
  it("escrows payout, emits BountyPosted, sets deadlines", async () => {
    ...
    const raceDur = 600;
    const tx = await env.hunt
      .connect(env.client)
      .postBounty(codeRoot, [], raceDur, { value: payout });
    const rcpt = await tx.wait();
    const ev = rcpt.logs.find((l) => {
      try {
        return env.hunt.interface.parseLog(l).name === "BountyPosted";
      } catch {
        return false;
      }
    });
    ...
    assert.equal(
      BigInt(b.settleDeadline) - BigInt(b.raceDeadline),
      24n * 3600n,
    );

    // Escrow held on contract
    assert.equal(
      await env.ethers.provider.getBalance(await env.hunt.getAddress()),
      payout,
    );
  });
  ...
  it("inScopeCwes empty means any class accepted (no revert when storing)", async () => {
    ...
    await env.hunt
      .connect(env.client)
      .postBounty(codeRoot, [], 600, { value: E(env, "0.01") });
...
describe("Hunt — postBounty validation", () => {
  it("reverts on zero payout", async () => {
    ...
    await assert.rejects(
      env.hunt
        .connect(env.client)
        .postBounty(randomRoot(env.ethers), [], 600, { value: 0 }),
      /payout=0/,
    );
  });
  ...
  it("reverts on empty codeRoot", async () => {
    ...
    await assert.rejects(
      env.hunt
        .connect(env.client)
        .postBounty(ZERO_ROOT, [], 600, { value: E(env, "0.01") }),
      /empty code/,
    );
  });
  ...
  it("reverts when raceDuration below MIN_RACE_DURATION", async () => {
    ...
    await assert.rejects(
      env.hunt
        .connect(env.client)
        .postBounty(randomRoot(env.ethers), [], 299, { value: E(env, "0.01") }),
      /race duration/,
    );
  });
  ...
  it("reverts when raceDuration above MAX_RACE_DURATION", async () => {
    ...
    await assert.rejects(
      env.hunt
        .connect(env.client)
        .postBounty(randomRoot(env.ethers), [], tooLong, {
          value: E(env, "0.01"),
        }),
      /race duration/,
    );
...
describe("Hunt — submitFinding happy path", () => {
  it("records finding, emits FindingSubmitted, increments totalSubmissions", async () => {
    ...
    const input = makeFindingInput({
      cweClass: cweHash(env.ethers, "SWC-107-reentrancy"),
    });
    ...
    const tx = await env.hunt
      .connect(env.user)
      .submitFinding(bountyId, hunterId, signed);
    const rcpt = await tx.wait();
    const ev = rcpt.logs.find((l) => {
      try {
        return env.hunt.interface.parseLog(l).name === "FindingSubmitted";
      } catch {
        return false;
      }
    });
    ...
    assert.equal(
      findings[0].hunter.toLowerCase(),
      env.user.address.toLowerCase(),
    );
  ...
  it("multiple findings from different hunters accumulate with monotonic indices", async () => {
    ...
    const { hunterId: h1 } = await mintHunterFor(env, env.third, {
      githubHandleHash: env.ethers.keccak256(
        env.ethers.toUtf8Bytes("hunter-third"),
      ),
    });

    const sigA = await buildSignedFindingInput(
      env,
      bountyId,
      h0,
      env.teeSigner,
    ...
    const sigB = await buildSignedFindingInput(
      env,
      bountyId,
      h1,
      env.teeSigner,
...
describe("Hunt — submitFinding validation", () => {
  it("reverts on bounty not found", async () => {
    ...
    const signed = {
      ...input,
      teeTimestamp: 1n,
      attestationSig: "0x" + "00".repeat(65),
    };
    await assert.rejects(
  ...
  it("reverts when bounty not Open (settled)", async () => {
    ...
    await env.hunt.connect(env.client).settleBounty(bountyId, 0, {
      severityCalibration: 4,
      precision: 4,
      coverage: 4,
      exploitability: 4,
    });

    const signed2 = await buildSignedFindingInput(
      env,
      bountyId,
      hunterId,
      env.teeSigner,
  ...
  it("reverts when teeTimestamp before postedAt", async () => {
    ...
    const input = makeFindingInput({
      teeTimestamp: BigInt(bounty.postedAt) - 1n,
    });
    const signed = await buildSignedFindingInput(
      env,
      bountyId,
  ...
  it("reverts when teeTimestamp after raceDeadline", async () => {
    ...
    const bounty = await env.hunt.getBounty(bountyId);
    const input = makeFindingInput({
      teeTimestamp: BigInt(bounty.raceDeadline) + 1n,
    });
    const signed = await buildSignedFindingInput(
      env,
      bountyId,
  ...
  it("reverts when cweClass not in non-empty inScope list", async () => {
    ...
    const input = makeFindingInput({
      cweClass: cweHash(env.ethers, "SWC-115-tx-origin"),
    });
    const signed = await buildSignedFindingInput(
      env,
      bountyId,
  ...
  it("reverts when self-eval average below MIN_FINDING_QUALITY_BPS", async () => {
    ...
    const input = makeFindingInput({
      severityCalibrationBps: 5000,
      precisionBps: 5000,
      coverageBps: 5000,
      exploitabilityBps: 5000,
    });
    const signed = await buildSignedFindingInput(
      env,
      bountyId,
...
describe("Hunt — settleBounty happy path", () => {
  it("settles, pays hunter, updates class rep + hunter aggregate, emits events", async () => {
    const env = await deployHunt();
    const { hunterId, bountyId, payout, cweClass } =
      await bountyWithFinding(env);
    ...
    const settled = rcpt.logs.find((l) => {
      try {
        return env.hunt.interface.parseLog(l).name === "BountySettled";
      } catch {
        return false;
      }
    });
    ...
    const repEv = rcpt.logs.find((l) => {
      try {
        return env.hunt.interface.parseLog(l).name === "ClassRepUpdated";
      } catch {
        return false;
      }
    });
...
describe("Hunt — v1.1 ClassRep math (submissions counted per accepted finding)", () => {
  it("submitFinding alone increments ClassRep.submissions before any settle", async () => {
    const env = await deployHunt();
    const { hunterId, bountyId } = await huntScenario(env);
    ...
    const rep = await env.hunt.getClassRep(hunterId, cls);
    assert.equal(rep.wins, 0n);
    assert.equal(rep.submissions, 1n);
    assert.equal(rep.totalEarnedWei, 0n);
    ...
    const repArgs = env.hunt.interface.parseLog(repEv).args;
    assert.equal(repArgs.hunterId, hunterId);
    assert.equal(repArgs.cweClass, cls);
    assert.equal(repArgs.wins, 0n);
    assert.equal(repArgs.submissions, 1n);
  });

  it("multiple submissions same hunter same CWE accumulate", async () => {
    const env = await deployHunt();
    const { hunterId, bountyId } = await huntScenario(env);
    const cls = cweHash(env.ethers, "SWC-107-reentrancy");

    for (let i = 0; i < 3; i++) {
      const root = "0x" + (i + 1).toString(16).padStart(2, "0").repeat(32);
      const input = makeFindingInput({ cweClass: cls, findingRoot: root });
      const signed = await buildSignedFindingInput(
        env,
        bountyId,
        hunterId,
    ...
    const rep = await env.hunt.getClassRep(hunterId, cls);
    assert.equal(rep.wins, 0n);
    assert.equal(rep.submissions, 3n);
  });

  it("losing hunter still has submissions++ after winner is settled", async () => {
    const env = await deployHunt();
    const { hunterId: hWinner, bountyId, payout } = await huntScenario(env);
    const { hunterId: hLoser } = await mintHunterFor(env, env.third, {
      githubHandleHash: env.ethers.keccak256(
        env.ethers.toUtf8Bytes("hunter-loser"),
      ),
    });
    const cls = cweHash(env.ethers, "SWC-107-reentrancy");

    const sigW = await buildSignedFindingInput(
      env,
      bountyId,
      hWinner,
      env.teeSigner,
      makeFindingInput({ cweClass: cls, findingRoot: "0x" + "aa".repeat(32) }),
    );
    await env.hunt.connect(env.user).submitFinding(bountyId, hWinner, sigW);

    const sigL = await buildSignedFindingInput(
      env,
      bountyId,
      hLoser,
      env.teeSigner,
      makeFindingInput({ cweClass: cls, findingRoot: "0x" + "bb".repeat(32) }),
    );
    await env.hunt.connect(env.third).submitFinding(bountyId, hLoser, sigL);
    ...
    const repW = await env.hunt.getClassRep(hWinner, cls);
    const repL = await env.hunt.getClassRep(hLoser, cls);

    assert.equal(repW.wins, 1n);
    assert.equal(repW.submissions, 1n);
    assert.equal(repW.totalEarnedWei, BigInt(payout));

    assert.equal(repL.wins, 0n);
    assert.equal(repL.submissions, 1n);
    assert.equal(repL.totalEarnedWei, 0n);
  });

  it("totalEarnedWei accumulates payouts that would overflow uint64", async () => {
    const env = await deployHunt();
    // uint64 max ≈ 18.446744 ether. A 20-ether payout would have wrapped under the old type.
    const bigPayout = env.ethers.parseEther("20");
    const { hunterId, bountyId } = await huntScenario(env, {
      payout: bigPayout,
    });
    const cls = cweHash(env.ethers, "SWC-107-reentrancy");
    ...
    const rep = await env.hunt.getClassRep(hunterId, cls);
    const h = await env.hunt.getHunter(hunterId);
    assert.equal(rep.totalEarnedWei, bigPayout);
    assert.equal(h.totalEarnedWei, bigPayout);
    // Sanity: confirm the value is genuinely above the legacy uint64 ceiling.
    assert.ok(bigPayout > (1n << 64n) - 1n, "expected payout > uint64.max");
  });
});
...
describe("Hunt — expireBounty", () => {
  it("refunds poster when no findings + race deadline passed", async () => {
    ...
    const posterBefore = await env.ethers.provider.getBalance(
      env.client.address,
    );
    const tx = await env.hunt.connect(env.third).expireBounty(bountyId);
    await tx.wait();
    const posterAfter = await env.ethers.provider.getBalance(
      env.client.address,
    );
  ...
  it("reverts when race not over and no findings yet", async () => {
    ...
    await assert.rejects(
      env.hunt.connect(env.third).expireBounty(bountyId),
      /still active/,
    );
  });
...
describe("Hunt — views", () => {
  ...
  it("classAvg returns zeros for an unused class", async () => {
    ...
    const avg = await env.hunt.classAvg(
      hunterId,
      cweHash(env.ethers, "signature-replay"),
    );
  ...
  it("totalHunters + totalBounties increment", async () => {
    ...
    await env.hunt
      .connect(env.third)
      .postBounty(randomRoot(env.ethers), [], 600, { value: E(env, "0.01") });
    assert.equal(await env.hunt.totalHunters(), 2n);
