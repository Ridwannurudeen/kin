...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--file')          out.file = argv[++i];
      ...
      console.error('usage: node scripts/post_bounty.js [--file path.sol] [--payout 0.05] [--race-duration 600] [--cwes a,b,c]');
      process.exit(2);
    } else {
...
async function loadOrGenerateNetworkKey(filePath) {
  try {
    ...
    console.error(`[post_bounty] generated new network key at ${filePath} (mode 0o600).`);
    console.error('[post_bounty] SHARE THIS FILE with every hunter daemon — they cannot');
    console.error('[post_bounty] decrypt the bounty code without it. Out-of-band channel only.');
    return buf;
  }
}
...
console.log(`race:      ${args.raceDuration}s (${(args.raceDuration / 60).toFixed(1)}m)`);
console.log(`cwes:      ${args.cwes}`);

// Validate race-duration against on-chain bounds before doing any storage work.
const [minRace, maxRace] = await Promise.all([hunt.MIN_RACE_DURATION(), hunt.MAX_RACE_DURATION()]);
if (BigInt(args.raceDuration) < minRace || BigInt(args.raceDuration) > maxRace) {
  console.error(`race duration ${args.raceDuration}s outside [${minRace},${maxRace}]`);
  process.exit(1);
}
...
if (bal < payoutWei + bufferWei) {
  console.error(`balance ${ethers.formatEther(bal)} OG < payout ${args.payout} + 0.02 buffer; refusing to post`);
  process.exit(1);
}

// Map CWE strings → bytes32 (throws on unknown).
const cweStrings = args.cwes.split(',').map(s => s.trim()).filter(Boolean);
if (cweStrings.length === 0) { console.error('no CWE classes provided'); process.exit(1); }
let cweBytes32;
try {
...
console.log(`source:    ${sourcePath} (${sourceText.length} bytes, wrapped to ${blob.length} bytes)`);
...
console.log(`encrypted: ${encrypted.length} bytes (AES-256-GCM, IV+tag prefixed)`);

// Upload to 0G Storage.
console.log('uploading to 0G Storage...');
const { rootHash: codeRoot, txHash: uploadTxHash } = await uploadRaw(encrypted, poster);
console.log(`codeRoot:  ${codeRoot}`);
console.log(`uploadTx:  ${uploadTxHash}`);

// Post the bounty.
console.log('posting bounty on-chain...');
const tx = await hunt.postBounty(codeRoot, cweBytes32, args.raceDuration, { value: payoutWei });
...
for (const log of rcpt.logs) {
  try {
    ...
    if (parsed?.name === 'BountyPosted') { bountyId = parsed.args.bountyId; break; }
  } catch {}
}
...
console.log(`raceDeadline:    ${new Date(raceDeadline * 1000).toISOString()} (${args.raceDuration / 60}m from now)`);
console.log(`settleDeadline:  ${new Date(settleDeadline * 1000).toISOString()} (raceDeadline + 24h)`);
