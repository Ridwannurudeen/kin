...
import { uploadRaw, downloadRaw, downloadEncryptedRecord } from '../lib/storage.js';
...
if (!PK) { console.error('PRIVATE_KEY required (funder)'); process.exit(1); }
...
const hunterWallets = JSON.parse(await fs.readFile('demo/.hunter-wallets.json', 'utf8'));
...
console.log(`bounty:    poster=${bounty.poster} payout=${ethers.formatEther(bounty.maxPayout)} OG ` +
            `raceDeadline=${new Date(Number(bounty.raceDeadline) * 1000).toISOString()}`);
const evt = (await huntRead.queryFilter(huntRead.filters.BountyPosted(BOUNTY_ID), -10000))[0];
if (!evt) { console.error('no BountyPosted event found in last 10000 blocks'); process.exit(1); }
...
async function runHunter(personaId) {
  ...
  console.log(`\n[${personaId}] starting (hunterId=${w.hunterId}, wallet=${operator.address.slice(0,10)}…)`);

  try {
    const result = await processBounty({
      hunt, provider, operator, teeSigner, sampleKey, networkKey,
      storage, invokeLLM,
      hunter, hunterId: BigInt(w.hunterId),
      bounty, bountyId: BOUNTY_ID,
...
console.log('\n=== RACE START ===');
const results = await Promise.all(Object.keys(hunterWallets).map(runHunter));

console.log('\n=== RACE END ===');
for (const r of results) {
  if (r.ok) {
    console.log(`  ✓ ${r.personaId} → ${r.cweClass} severity=${r.severity} tx=${r.txHash} attempts=${r.attempts}`);
  } else {
...
if (findingsCount > 0n) {
  ...
  for (let i = 0; i < findings.length; i++) {
    const f = findings[i];
    console.log(`  finding[${i}]: hunter#${f.hunterId} cwe=${f.cweClass.slice(0,18)}… sev=${f.severity} teeTs=${f.teeTimestamp} tx submittedAt=${f.submittedAt}`);
  }
}
