...
import {
  uploadRaw, downloadRaw, downloadEncryptedRecord, decrypt,
} from '../lib/storage.js';
import {
  encryptToPubkey,
} from '../lib/ecdh.js';
...
import { CANONICAL_CWES, cweToBytes32, bytes32ToCwe, severityToUint } from '../lib/cwe.js';
...
function extractContractsInScope(codeBuffer) {
  ...
  try {
    ...
    if (obj && obj.files && typeof obj.files === 'object') return Object.keys(obj.files);
...
async function loadHunterNetworkKey({ envHex, filePath }) {
  if (envHex) {
    ...
    if (buf.length !== 32) throw new Error(`HUNTER_NETWORK_KEY wrong length: ${buf.length} (want 32)`);
    return buf;
  }
  try {
    const buf = await fs.readFile(filePath);
    if (buf.length !== 32) throw new Error(`${filePath} wrong length: ${buf.length} (want 32)`);
    return buf;
  } catch (e) {
...
export async function processBounty({
  hunt, provider, operator, teeSigner, sampleKey, networkKey,
  storage, invokeLLM, hunter, hunterId, bounty, bountyId, evtTxHash, logger = console.log,
}) {
  ...
  logger(`[bounty ${bountyId}] top-${top.length} retrieved (scores: ${top.map(t => t.score.toFixed(2)).join(',')})`);
  ...
  const brief = {
    ...
    focus: inScopeCweStrings.length ? inScopeCweStrings : CANONICAL_CWES.slice(),
    diff: codeText,
  ...
  for (attempts = 1; attempts <= MAX_RETRIES; attempts++) {
    try {
      ...
      lastError = e;
      logger(`[bounty ${bountyId}] attempt ${attempts} LLM error: ${e.message?.slice(0, 120)}, retrying`);
      continue;
    }
    if (reviewResult.selfEval.overallBps >= MIN_OUTPUT_QUALITY_BPS) break;
    logger(`[bounty ${bountyId}] attempt ${attempts} qualityScore ${reviewResult.selfEval.overallBps} below ${MIN_OUTPUT_QUALITY_BPS}, retrying`);
  }
  if (!reviewResult) {
    logger(`[bounty ${bountyId}] LLM failed ${MAX_RETRIES}x — falling back to local audit heuristic (lib/audit-fallback.js); last LLM error: ${lastError?.message?.slice(0, 120)}`);
    ...
    try {
      ...
      else if (obj && typeof obj === 'object' && !Array.isArray(obj)) codeFiles = obj;
      else codeFiles = { 'Code.sol': codeBuf.toString('utf8') };
    } catch { codeFiles = { 'Code.sol': codeBuf.toString('utf8') }; }
    reviewResult = localAuditFallback({
      codeFiles,
      specialty: hunter.specialty,
      inScopeCweStrings,
    });
    logger(`[bounty ${bountyId}] local fallback: ${reviewResult.findings.length} findings, overall ${reviewResult.selfEval.overallBps}bps, model=${reviewResult.modelName}`);
  }
  if (reviewResult.selfEval.overallBps < MIN_OUTPUT_QUALITY_BPS) {
    logger(`[bounty ${bountyId}] gave up after ${MAX_RETRIES} attempts; letting bounty race elapse`);
    return { ok: false, reason: 'quality-gate', attempts };
  }
  logger(`[bounty ${bountyId}] passed quality gate at attempt ${attempts}, overall ${reviewResult.selfEval.overallBps}bps; generated ${reviewResult.findings.length} findings`);

  // 7. Pick the best in-scope finding to submit.
  const best = pickBestFinding(reviewResult.findings, inScopeCweStrings);
  if (!best) {
    logger(`[bounty ${bountyId}] no in-scope findings (had ${reviewResult.findings.length}); skipping`);
    return { ok: false, reason: 'no-in-scope-finding', attempts };
  }
  logger(`[bounty ${bountyId}] generated ${reviewResult.findings.length} findings; best = ${best.finding.cweClass}/${best.finding.severity}`);
  ...
  try {
    ...
    logger(`[bounty ${bountyId}] unknown CWE class from model: ${best.finding.cweClass}; skipping`);
  ...
  const { rootHash: findingRoot } = await storage.uploadRaw(encryptedFinding, operator);
  ...
  if (teeTimestamp < bounty.postedAt || teeTimestamp > bounty.raceDeadline) {
    logger(`[bounty ${bountyId}] teeTimestamp ${teeTimestamp} outside [${bounty.postedAt},${bounty.raceDeadline}]; skipping`);
  ...
  const { sig: attestationSig } = await signFindingAttestation(teeSigner, attestParams);
  ...
  logger(`[bounty ${bountyId}] submitted finding idx ${best.index} tx ${tx.hash} block ${rcpt.blockNumber}`);
...
async function main() {
  ...
  const HUNTER_IDS = (process.env.HUNTER_IDS || '').split(',').filter(Boolean).map(s => BigInt(s.trim()));
  const POLL_MS = Number(process.env.POLL_MS || 8000);
  const SAMPLE_KEY_PATH = process.env.SAMPLE_KEY_PATH || '.user-key.bin';
  const NETWORK_KEY_PATH = process.env.HUNTER_NETWORK_KEY_PATH || '.hunter-network-key.bin';
  ...
  if (HUNTER_IDS.length === 0) { console.error('HUNTER_IDS required (comma-separated)'); process.exit(1); }
  ...
  if (!networkKey) {
    console.warn(`[hunter] no HUNTER_NETWORK_KEY env or ${NETWORK_KEY_PATH} file; bounties will be skipped`);
  }
  ...
  for (const id of HUNTER_IDS) {
    ...
    if (h.owner.toLowerCase() !== operator.address.toLowerCase()) {
      console.error(`[hunter] hunter ${id} owned by ${h.owner}, not us — skipping`);
      continue;
    }
    if ((await hunt.teeSigner()).toLowerCase() !== teeSigner.address.toLowerCase()) {
      console.error(`[hunter] TEE_SIGNER_PRIVATE_KEY address ${teeSigner.address} != on-chain ${await hunt.teeSigner()}`);
      process.exit(1);
    }
    huntersCache.set(id, h);
    console.log(`[hunter] tracking hunter #${id} | specialty "${h.specialty}"`);
  }
  if (huntersCache.size === 0) { console.error('[hunter] no owned hunters, exiting'); process.exit(1); }
  ...
  if (!providerAddr) { console.error('[hunter] no inference providers found'); process.exit(1); }
  ...
  try {
    ...
    console.error(`[hunter] another daemon already running (lockfile ${lockPath} exists). exit.`);
    process.exit(1);
  }
  ...
  while (true) {
    try {
      ...
      if (currentBlock > lastBlock) {
        ...
        const events = await hunt.queryFilter(filter, lastBlock + 1, currentBlock);
        for (const evt of events) {
          ...
          if (!networkKey) {
            console.warn(`[bounty ${bountyId}] no network key loaded; skipping`);
            continue;
          }
          ...
          processBounty({
            hunt, provider, operator, teeSigner, sampleKey, networkKey,
            storage, invokeLLM,
            hunter: activeHunter, hunterId: activeHunterId,
            bounty: await hunt.getBounty(bountyId), bountyId,
            evtTxHash: evt.transactionHash,
          }).catch(e => console.error(`[bounty ${bountyId}] failed:`, e.message));
        }
