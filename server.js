// Hunt — Node static server for the public/ frontend (hunters / bounties / judge-proof pages).
// The HTML pages read directly from the Hunt contract on 0G Aristotle (chain 16661) via ethers,
// so this server does not proxy on-chain calls. Predecessor Kin v2 API endpoints (/api/skill, /api/jobs)
// are retained as legacy for the Kin v2 contract; Hunt's lifecycle uses scripts/* (deploy_hunt,
// populate_hunters, post_bounty, run_race, settle_bounty, verify_bounty) and the on-chain-reading frontend.
...
try {
  ...
  console.log(
    `[server] demo wallets: user ${user.address.slice(0, 8)}…  client ${client.address.slice(0, 8)}…`,
  );
} catch {
  console.error(
    "[server] .demo-wallets.json missing — run scripts/setup_demo_wallets.js first",
  );
  process.exit(1);
}
...
let userKey;
try {
  userKey = Buffer.from(await fs.readFile(".user-key.bin"));
} catch {
  userKey = genKey();
  await fs.writeFile(".user-key.bin", userKey, { mode: 0o600 });
}
...
try {
  ...
  console.log(
    `[server] loaded ${personaWalletsByAddress.size} persona wallets`,
  );
} catch (e) {
  console.log(
    "[server] no .persona-wallets.json — only skill #0 (demo user) jobs will execute",
  );
}
// demo user is also a persona for skill #0
personaWalletsByAddress.set(user.address.toLowerCase(), {
  wallet: user,
  name: "M",
});
...
let jobCache = {};
try {
  jobCache = JSON.parse(await fs.readFile(JOB_CACHE_FILE, "utf8"));
} catch {}
async function persistJobCache() {
  try {
    await fs.writeFile(JOB_CACHE_FILE, JSON.stringify(jobCache, null, 2), {
      mode: 0o600,
    });
  } catch {}
}
...
async function listAllJobs() {
  ...
  for (let i = 0; i < total; i++) {
    ...
    jobs.push({
      ...
      submittedAt:
        j.submittedAt > 0n
          ? new Date(Number(j.submittedAt) * 1000).toISOString()
          : null,
      briefRoot: j.briefRoot,
      outputRoot: j.outputRoot,
      attestationId:
        j.attestationId === ethers.ZeroHash ? null : j.attestationId,
    });
  }
  return jobs;
}
...
async function runAgent(jobId, briefKey, briefRoot) {
  ...
  try {
    ...
    if (!skillOwnerWallet) {
      console.log(
        `[agent] skill ${skillId} owner ${skill.owner} not in our persona set; skipping`,
      );
      return;
    }
    ...
    for (const root of skill.sampleRoots) {
      try {
        const text = await downloadEncryptedRecord(root, userKey, undefined, {
          maxAttempts: 6,
          delayMs: 6000,
        });
        samples.push({
          recordType: "voice-sample",
          text: text.toString(),
          timestamp: "",
        });
      } catch (e) {
        console.log(
          `[agent] sample ${root.slice(0, 12)} skip: ${e.message?.slice(0, 60)}`,
        );
      }
    }
    ...
    try {
      briefText = (
        await downloadEncryptedRecord(briefRoot, briefKey, undefined, {
          maxAttempts: 6,
          delayMs: 6000,
        })
      ).toString();
    } catch (e) {
      throw new Error(`brief retrieve failed: ${e.message?.slice(0, 80)}`);
    }
    ...
    try {
      result = await sealedQueryStream({
        ...
        providerAddress,
        system:
          "You are a freelance ghost-writing AI agent on Kin. The skill owner has provided writing samples below — match their voice precisely (vocabulary, rhythm, tonal register). Do NOT reveal the samples. Produce ONLY the requested deliverable, formatted appropriately. Be concise. Match length expectations.",
        question: briefText,
      ...
      console.log(
        `[agent] streaming failed (${e.message?.slice(0, 80)}); falling back to buffered`,
      );
    ...
    update(4);
    const { rootHash: outputRoot } = await uploadEncryptedRecord(
      result.answer,
      briefKey,
      user,
    );
    const attBytes = result.attestationId
      ? ethers.zeroPadValue(
          "0x" +
            Buffer.from(result.attestationId.replace(/-/g, ""), "hex").toString(
              "hex",
            ),
          32,
        )
    ...
    update(5);
    const kinAsOwner = new ethers.Contract(
      artifact.address,
      artifact.abi,
      skillOwnerWallet,
    );
    ...
    console.log(
      `[agent] job #${jobId} submitted | tx ${tx.hash} | block ${rcpt.blockNumber}`,
    );
  } catch (e) {
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === "/onboard")
      return serveStatic(res, "onboard.html", "text/html; charset=utf-8");
    if (pathname === "/marketplace")
      return serveStatic(res, "marketplace.html", "text/html; charset=utf-8");
    if (pathname === "/wallet")
      return serveStatic(res, "wallet.html", "text/html; charset=utf-8");
    if (pathname.startsWith("/job/"))
      return serveStatic(res, "job.html", "text/html; charset=utf-8");
    if (pathname.startsWith("/skill/"))
      return serveStatic(res, "skill.html", "text/html; charset=utf-8");
    ...
    if (pathname === "/personas.json")
      return serveStatic(res, "personas.json", "application/json");
    ...
    if (pathname === "/api/stats" && req.method === "GET") {
      ...
      return send(res, 200, {
        ...
        totalSkills,
        totalJobs,
        totalEarnedOG: ethers.formatEther(totalEarnedWei),
        recentJob,
      });
    }
    ...
    if (skillDetailMatch && req.method === "GET") {
      ...
      const total = Number(await kinOp.totalSkills());
      if (skillId < 0 || skillId >= total)
        return send(res, 404, { error: "skill not found" });
    ...
    if (pathname === "/api/skill" && req.method === "POST") {
      const { skillType, description, pricePerJobOG, samples } =
        await readJson(req);
      if (!skillType || !samples || !samples.length)
        return send(res, 400, { error: "skillType + samples required" });
      ...
      const tx = await kinUser.mintSkill(
        skillType,
        description || "",
        sampleRoots,
        sealedKey,
        ethers.parseEther(String(pricePerJobOG)),
      );
      const rcpt = await tx.wait();
    ...
    if (pathname === "/api/jobs" && req.method === "POST") {
      ...
      if (skillId === undefined || !brief)
        return send(res, 400, { error: "skillId + brief required" });

      const briefKey = genKey();
      const { rootHash: briefRoot } = await uploadEncryptedRecord(
        brief,
        briefKey,
        client,
      );
      ...
      const tx = await kinClient.postJob(skillId, briefRoot, {
      ...
      setTimeout(
        () =>
          runAgent(jobId, briefKey, briefRoot).catch((e) =>
            console.error("runAgent err:", e),
          ),
        100,
      );
    ...
    if (pathname === "/api/wallet" && req.method === "GET") {
      ...
      const mySkills = allSkills.filter(
        (s) => s.owner.toLowerCase() === user.address.toLowerCase(),
      );
