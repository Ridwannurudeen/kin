...
function buildBriefBlock(brief) {
  const scheduleLine = brief.bountySchedule
    ? Object.entries(brief.bountySchedule).filter(([, v]) => v).map(([k, v]) => `${k}=${v}`).join(', ')
    : '';
  const fields = [
    ...
    brief.priorAudits?.length       ? `priorAudits: ${brief.priorAudits.join('; ')}` : null,
...
export function parseReviewResponse(raw) {
  ...
  if (!Array.isArray(obj.findings)) throw new Error('missing or non-array findings field');
  ...
  const overallBps = Math.round(
    (severityCalibrationBps + precisionBps + coverageBps + exploitabilityBps) / 4,
  );
  const rationale = typeof se.rationale === 'string' ? se.rationale : '';

  return {
    findings,
    selfEval: { severityCalibrationBps, precisionBps, coverageBps, exploitabilityBps, overallBps, rationale },
  };
}
