...
//
// Used because Kin v2 does not store wallet pubkeys on-chain (they're implicit in every
...
export async function pubkeyFromTx(provider, txHash) {
  ...
  if (recoveredAddr.toLowerCase() !== tx.from.toLowerCase()) {
    throw new Error(`pubkey recovery mismatch: got ${recoveredAddr}, expected ${tx.from}`);
  }
  return pubkey;
}
