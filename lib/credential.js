// Shared signing primitives for Kin v2. Used by:
...
export function credentialDigest(wallet, cred) {
  return ethers.keccak256(ABI.encode(
    ...
    [wallet, cred.githubHandleHash, cred.accountAgeDays, cred.mergedPRs,
     cred.codeReviewCount, cred.verifiedAt, cred.verifier],
  ));
}
...
export function fingerprintDigest(sampleRoots, fp) {
  return ethers.keccak256(ABI.encode(
    ['bytes32[]', 'uint16', 'uint16', 'uint16', 'uint16', 'uint16', 'bytes32'],
    [sampleRoots, fp.vocabEntropyBps, fp.domainTermBps, fp.structuralBps,
     fp.specificityBps, fp.overallBps, fp.modelDigest],
  ));
}
...
export function attestationDigest(jobId, outputRoot, qualityScore, modelDigest) {
  return ethers.keccak256(ABI.encode(
...
export async function signAttestation(teeSigner, jobId, outputRoot, qualityScore, modelDigest) {
  const digest = attestationDigest(jobId, outputRoot, qualityScore, modelDigest);
  const sig = await teeSigner.signMessage(ethers.getBytes(digest));
  return { digest, sig };
}
...
export function findingDigest(params) {
  return ethers.keccak256(ABI.encode(
    ...
    [params.bountyId, params.codeRoot, params.hunterId, params.cweClass,
     params.severity, params.findingRoot, params.modelDigest, params.teeTimestamp,
