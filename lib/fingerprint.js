...
// structured fingerprint with a Kin-operated signing key. v3 = wire 0G's per-response
...
export function buildFingerprintPrompt(samples) {
  return [
    ...
    { role: 'user',   content: `Score these ${samples.length} samples:\n\n${buildBatchText(samples)}` },
  ];
}
...
function extractJson(raw) {
  ...
  if (firstBrace === -1 || lastBrace === -1) throw new Error(`no JSON object in response: ${raw.slice(0, 120)}`);
...
export function computeOverallBps({ vocabEntropyBps, domainTermBps, structuralBps, specificityBps }) {
  const agg = 0.2 * vocabEntropyBps + 0.4 * domainTermBps + 0.2 * structuralBps + 0.2 * specificityBps;
  return clampBps(agg);
}
...
export function parseFingerprintResponse(raw) {
  ...
  const overallBps      = computeOverallBps({ vocabEntropyBps, domainTermBps, structuralBps, specificityBps });
  const rationale       = typeof obj.rationale === 'string' ? obj.rationale : '';
  return { vocabEntropyBps, domainTermBps, structuralBps, specificityBps, overallBps, rationale };
}
...
export function fingerprintModelDigest(modelName) {
  return ethers.keccak256(ethers.toUtf8Bytes(`${modelName}|${FINGERPRINT_MODEL_VERSION}`));
}
...
export function localFingerprint(samples) {
  ...
  const DOMAIN = new Set([
    'race','condition','toctou','mutex','lock','deadlock','reentrancy','overflow','underflow',
    'null','undefined','panic','unwrap','await','async','closure','borrow','lifetime','arc','rc',
    'atomic','channel','spawn','retry','timeout','swallow','log','assert','invariant','allocation',
    'leak','reference','pointer','cache','toctou','validator','token','session','auth','middleware',
    'cors','xss','csrf','sql','injection','sanitize','encode','escape','hash','signature','nonce',
    'replay','revert','require','calldata','storage','slot','proxy','upgrade','attack','exploit',
    'audit','review','test','mock','stub','regex','backtrack','catastrophic','o(n)','complexity',
    'idiom','convention','refactor','rename','rebase','merge','conflict','typo','comment',
    // Hunt audit-domain extensions: oracle, access-control, upgrade-safety, DeFi primitives.
    'oracle','manipulation','twap','chainlink','pyth','sequencer','staleness','stale','heartbeat',
    'sandwich','frontrun','slippage','liquidation','liquidate','collateral','ltv','ratio',
    'governance','multisig','timelock','dao','modifier','onlyowner','onlyadmin','ownable',
    'initializer','disableinitializers','delegatecall','selfdestruct','uups','initializable',
    'rolemanager','grantrole','revokerole','accesscontrol','permit','allowance','approval',
    'flashloan','vault','share','withdraw','deposit','mint','burn','transferfrom','transfer',
    'callback','hook','beforewithdraw','aftertransfer','erc20','erc721','erc777','erc1155',
    'pausable','pause','sweep','rescue','keeper','liveness','censorship','grief','dos',
    'precision','rounding','divisionbyzero','underflow','arithmetic','math','wad','ray',
    'ecrecover','recover','digest','typehash','eip712','eip191','eip2612','chainid',
    'msgsender','txorigin','externalcall','lowlevel','statictransfer','callreturndata',
    'invariantviolation','tvl','apr','apy','interest','fee','tax','bps','basisp','wei',
    'block','timestamp','number','gasleft','gaslimit','outofgas','dust','rounding',
  ]);
  let domainHits = 0;
  ...
  const variance = lens.reduce((a, b) => a + (b - mean) ** 2, 0) / Math.max(lens.length, 1);
  ...
  const codeRefs = (text.match(/[a-zA-Z_][a-zA-Z0-9_]*[\.:][a-zA-Z_][a-zA-Z0-9_]*|line\s*\d+|src\/|\b[a-zA-Z_][a-zA-Z0-9_]{2,}\(|::[a-zA-Z_]|`[^`]+`|<[A-Z][A-Za-z0-9_<>,\s]*>/g) || []).length;
  const refsPerSentence = codeRefs / Math.max(sentences.length, 1);
  const specificityBps = clampBps(10000 * (1 - Math.exp(-refsPerSentence * 1.5)));

  const overallBps = computeOverallBps({ vocabEntropyBps, domainTermBps, structuralBps, specificityBps });
  return {
    vocabEntropyBps, domainTermBps, structuralBps, specificityBps, overallBps,
    rationale: '0G Sealed Inference unavailable; scored locally via lexical+structural feature stats over the samples.',
  };
}
...
///   teeSigner   — ethers Wallet whose address matches Kin.teeSigner() on-chain
///
/// Returns: { fingerprint, attestationId, rationale }
export async function fingerprintSamples({ invokeLLM, samples, sampleRoots, teeSigner }) {
  if (samples.length !== sampleRoots.length) throw new Error('samples/sampleRoots length mismatch');
  if (samples.length < 3 || samples.length > 20) throw new Error('samples 3..20');
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      console.error(`[fingerprint] attempt ${attempt}/${MAX_ATTEMPTS} failed: ${e.message}`);
      if (result) {
        console.error(`  model: ${result.model || 'n/a'} | attestation: ${result.attestationId || 'n/a'} | answer length: ${(result.answer || '').length}`);
      }
      if (attempt === MAX_ATTEMPTS) {
        console.error('[fingerprint] all LLM attempts failed — falling back to local feature-stats fingerprint (V2_SPEC §14)');
  ...
  const modelDigest = usedFallback
    ? ethers.keccak256(ethers.toUtf8Bytes('kin-local-stats|' + FINGERPRINT_MODEL_VERSION))
